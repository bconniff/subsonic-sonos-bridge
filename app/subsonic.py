...
def _filter_results(req: SearchRequest, res, to_infos_fn = lambda x: x):
    infos = [ to_infos_fn(it) for it in (res or []) ]
...
class SubsonicAPI:
    ...
    async def _search_songs_by_starred(self, req: SearchRequest) -> list[SongInfo]:
        logger.info('searching songs by starred')
        res = await self.conn.get_starred2()
        return _filter_results(req, res.song, _to_song_info)

    async def _search_albums_by_starred(self, req: SearchRequest) -> list[AlbumInfo]:
        logger.info('searching albums by starred')
        res = await self.conn.get_starred2()
        return _filter_results(req, res.album, _to_album_info)
    ...
    async def _search_playlists_by_query(self, req: SearchRequest) -> list[PlaylistInfo]:
        ...
        return _filter_results(req, result, _to_playlist_info)

    async def _search_by_id(self, req: SearchRequest, fetch_fn, to_infos_fn = lambda x: x):
        logger.info('searching by specific item id')
        items = []
        try:
            result = await fetch_fn()
            items = to_infos_fn(result)
        except DataNotFoundError:
            pass
        return _filter_results(req, items)

    async def _search_playlists_by_id(self, req: SearchRequest) -> list[PlaylistInfo]:
        return await self._search_by_id(
            req,
            lambda: self.conn.get_playlist(req.playlist_id),
            lambda res: [ _to_playlist_info(res) ]
        )

    async def _search_albums_by_id(self, req: SearchRequest) -> list[AlbumInfo]:
        return await self._search_by_id(
            req,
            lambda: self.conn.get_album(req.album_id),
            lambda res: [ _to_album_info(res) ]
        )

    async def _search_songs_by_playlist_id(self, req: SearchRequest) -> list[SongInfo]:
        return await self._search_by_id(req, lambda: self._get_playlist(req.playlist_id))

    async def _search_songs_by_album_id(self, req: SearchRequest) -> list[SongInfo]:
